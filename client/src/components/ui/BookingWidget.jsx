...
const BookingWidget = ({ place }) => {
  ...
  const handleBooking = async () => {
    ...
    if (numberOfNights < 1) {
      ...
      return toast.error("No. of units can't be less than 1");
    ...
    try {
      ...
      toast('Congratulations! You have booked your slot.');
    } catch (error) {
  ...
  return (
    <div className="rounded-2xl bg-white p-4 shadow-xl">
      <div className="text-center text-xl">
        Price: <span className="font-semibold">₹{place.price}</span> / per unit (Litre)
      </div>
